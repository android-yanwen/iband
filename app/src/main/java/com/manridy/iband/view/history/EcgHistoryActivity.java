...
public class EcgHistoryActivity extends BaseActionActivity {
    ...
    protected void initView(Bundle savedInstanceState) {
        setContentView(R.layout.activity_history_ecg);
...
//                showShare();
    ...
    public void onMainEvent(EventMessage event){
        ...
        }else if(event.getWhat() == EventGlobal.REFRESH_VIEW_ECG_HISTORY){
            historyAdapter.setItemList(itemList);
        }
    }
    ...
    public void onBackgroundEvent(EventMessage event){
        if (event.getWhat() == EventGlobal.DATA_LOAD_HR_HISTORY) {
            ...
            itemList = IbandDB.getInstance().getMonthEcg(days);
            if (itemList.size()>0) {
                curItem = itemList.get(0);
                dataMin =Integer.valueOf(curItem.itemNum);
            }
            for (EcgHistoryAdapter.Item item : itemList) {
        ...
        }else if(event.getWhat() == EventGlobal.DATA_LOAD_ECG_HISTORY) {
            curItem = null;
            days = TimeUtil.getMonthToDay(mCalendar);
            itemList = IbandDB.getInstance().getMonthEcg(days);
            if (itemList.size()>0) {
...
//                dataAvg /= itemList.size();
//            }
            EventBus.getDefault().post(new EventMessage(EventGlobal.REFRESH_VIEW_ECG_HISTORY));
        }
    }
    ...
    private void initHistoryData(){
        switch (historyType) {
            ...
            case 2:
                ...
                break;
            case 3:
                EventBus.getDefault().post(new EventMessage(EventGlobal.DATA_LOAD_ECG_HISTORY));
                break;
        }
    }
